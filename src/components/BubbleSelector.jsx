...
export default function BubbleSelector({ label, options, value, onChange, compact = false }) {
  return (
    <div className={compact ? 'mb-2' : 'mb-4'}>
      {label && (
        <p className={`font-semibold mb-2 ${compact ? 'text-[10px] uppercase tracking-wider text-slate-700' : 'text-sm text-slate-800 dark:text-slate-200'}`}>
          {label}
        </p>
      )}
      <div className={compact ? 'space-y-0.5' : 'space-y-1.5'}>
        {options.map((opt) => {
          ...
          return (
            <label key={opt.value}
              className={`flex items-center gap-2 cursor-pointer group ${compact ? 'py-0.5' : 'py-1.5'}`}
              ...
              <span className={`flex-shrink-0 rounded-full border-2 transition-all duration-200 flex items-center justify-center
                ...
                {isSelected && <span className={`rounded-full bg-white ${compact ? 'w-1 h-1' : 'w-2 h-2'}`} />}
              </span>
